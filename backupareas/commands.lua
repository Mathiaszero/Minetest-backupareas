...
local function fix_lines(schema_dir, dir_list)
	-- for file_index, file in ipairs(dir_list) do
	-- 	--delete empty lines
	-- 	local schema_file, err = io.open(schema_dir..file, "a")
	-- 	local lines = schema_file:lines()
	...
	-- 	-- local lines = schema_file:lines()
	-- 	-- for line_index, line in ipairs(lines) do
	-- 	-- 	if line ~= nil and string.sub(line, -1) ~= "}"
	-- 	-- 		count = count + 1
	-- 	-- end
	-- 	-- schema_file:close()
	-- end
	...
	func = function(name, param)
		...
		for i, file in ipairs(dir_list) do
			...
			for line in schema_file:lines() do
				...
				local obj = core.deserialize(line)--ok
				if obj then
					--core.chat_send_player(name, obj[1])
					--local json = core.parse_json(obj)--string expected got table error
					--core.chat_send_player(name, dump(obj))
					...
					for key, value in pairs(node_meta_keys) do
						--core.chat_send_player(name, i..","..v)
				...
				-- if line then
				-- 	local obj = core.deserialize(line)
				-- 	core.chat_send_player(name, obj[1])
				-- end

				--local json = core.parse_json(obj)
				--core.chat_send_player(name, obj[1])
				-- local x = obj[1]
				-- local y = obj[2]
				-- local z = obj[3]
